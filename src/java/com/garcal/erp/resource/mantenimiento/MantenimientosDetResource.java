...
import com.garcal.erp.DAO.mantenimiento.MantenimientosDetDAO;
...
@Path("mantenimientosdet")

public class MantenimientosDetResource {
   
    @POST
    @Path("nuevo")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces({MediaType.APPLICATION_JSON})
    public Response postJson(MantenimientosDet mantenimientosdet) {
        ...
        ResponseAnswer postResponse = MantenimientosDetDAO.insert(mantenimientosdet);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces({MediaType.APPLICATION_JSON})
    public Response actualizarJson(MantenimientosDet mantenimientosdet) {
        /* ValidatorFactory factory = Validation.buildDefaultValidatorFactory();
         Validator validator = factory.getValidator();
         validator.validate(business);
         Set<ConstraintViolation<Business>> violations = validator.validate(business);
         ...
         if (violations.isEmpty()) {*/
        ResponseAnswer postResponse = MantenimientosDetDAO.update(mantenimientosdet);
        return Response.status(Response.Status.OK)
                .header("Access-Control-Allow-Origin", "*") // Headers for Swagger UI.
                .header("Access-Control-Allow-Headers", "Content-Type,X-Requested-With,Authorization")
                .header("Access-Control-Allow-Methods", "OPTIONS,POST")
